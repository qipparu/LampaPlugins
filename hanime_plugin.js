...
function HanimeComponent(componentObject) {
    ...
    this.build = function (result) {
     ...
     this.showCardContextMenu = function(cardElement, cardData) {
         ...
         if(window.Lampa && Lampa.Lang && typeof Lampa.Lang.translate === 'function') {
             menu_favorite = [
                 { title: Lampa.Lang.translate('title_book'), where: 'book', checkbox: true, checked: status.book }, // Запланировано (чекбокс)
                 { title: Lampa.Lang.translate('title_like'), where: 'like', checkbox: true, checked: status.like }, // Нравится (чекбокс)
                 { title: Lampa.Lang.translate('title_wath'), where: 'wath', checkbox: true, checked: status.wath }, // Смотрю (чекбокс)
                 { title: Lampa.Lang.translate('menu_history'), where: 'history', checkbox: true, checked: status.history }, // История (чекбокс)
                 { title: Lampa.Lang.translate('settings_cub_status'), separator: true } // Разделитель для статусов
                 // Добавьте сюда пункты для маркеров состояния ('look', 'viewed', 'scheduled', 'continued', 'thrown')
                 // по аналогии с standard Card.js onMenu методом.
             ];
         } else {
    ...
    this.fetchStreamAndMeta = function (id, meta) {
        ...
        Promise.all([
            ...
            if (streamData && streamData.streams && Array.isArray(streamData.streams) && streamData.streams.length > 0) {
                ...
                var finalStreamUrl = streamToPlay ? streamToPlay.url : null; // Получаем URL потока.
                ...
                if(finalStreamUrl) {
                     try {
                         ...
                         // Если домен содержит 'highwinds-cdn.com' и у нас есть PROXY_BASE_URL, оборачиваем.
                ...
                var playerObject = {
                    title: fullMetaData ? (fullMetaData.name || fullMetaData.title || 'Без названия') : 'Без названия', // Название для плеера.
                    url: finalStreamUrl, // URL потока для воспроизведения.
                    poster: fullMetaData ? (fullMetaData.poster || fullMetaData.background || '') : '', // URL постера для плеера.
                     // Дополнительные поля для плеера Lampa.
                     // duration: fullMetaData?.runtime ? fullMetaData.runtime * 60 : undefined, // Длительность в секундах (если runtime есть)
                ...
                if (playerObject.url && window.Lampa && Lampa.Player && typeof Lampa.Player.play === 'function' && typeof Lampa.Player.playlist === 'function') {
                     ...
                     Lampa.Player.playlist([playerObject]); // Устанавливаем текущий элемент как плейлист.
                     ...
                     if (fullMetaData && window.Lampa && Lampa.Favorite && typeof Lampa.Favorite.add === 'function') {
                            ...
                            Lampa.Favorite.add('history', historyMeta, 100); // Добавляем в историю.
    ...
    this.empty = function (msg) {
         ...
         if (window.Lampa && Lampa.Empty && typeof Lampa.Empty === 'function' && typeof Lampa.Empty.prototype.render === 'function') {
             ...
             if (typeof empty.start === 'function') this.start = empty.start;
              ...
              if(html && typeof html.empty === 'function' && typeof html.text === 'function') html.empty().text(msg + ' (Компонент Empty недоступен в Lampa)'); // Очищаем и добавляем текст сообщения.
    ...
    this.create = function () {
        console.log("HanimeComponent: create()");
         // Инициализируем Scroll и Network в начале создания компонента.
         if (!scroll && window.Lampa && typeof Lampa.Scroll === 'function') {
             scroll = new Lampa.Scroll({ mask: true, over: true, step: 250, direction: 'horizontal' });
    ...
    this.start = function () {
        ...
        // Выполняем все необходимые проверки на наличие Controller, Scroll и их методов ПЕРЕД использованием.
    ...
    this.pause = function () {
         ...
         // Проверяем, что Lampa.Controller доступен, что наш контроллер 'content' активен, и метод item() доступен.
...
function startPlugin() {
    ...
    function initializeLampaDependencies() {
         ...
         if (!window.Lampa || typeof window.Lampa !== 'object' || !Lampa.Template || typeof Lampa.Template !== 'object' || !Lampa.Component || typeof Lampa.Component !== 'object' || !Lampa.Activity || typeof Lampa.Activity !== 'object' || !Lampa.Controller || typeof Lampa.Controller !== 'object' || !window.$ || typeof window.$ !== 'function' || !Lampa.Scroll || typeof Lampa.Scroll !== 'function' || !Lampa.Reguest || typeof Lampa.Reguest !== 'function') {
              ...
              if(window.Lampa && Lampa.Noty && typeof Lampa.Noty.show === 'function') {
                 Lampa.Noty.show('Ошибка плагина: Компоненты Lampa недоступны. Обновите Lampa или плагин.', 15000); // Более длительное уведомление
              }
         ...
         // Вызывается в самом конце initializeLampaDependencies.
    ...
    function addMenuItem() {
         ...
         // Проверка на случай дублирования пункта меню по тексту (менее вероятно теперь).
    ...
    console.log("Hanime Plugin: startPlugin() invoked. Setting up Lampa ready listener.");
     ...
     if (window.plugin_hanime_catalog_ready) {
         ...
         return; // Если флаг уже установлен, выходим сразу.
     }
