 ...
 * Version: 1.2.1 (Fixed ReferenceError and template scope issue)
 */
                         ...
                         if (metaData && metaData.meta) {
                             ...
                             if (metaData.meta.description) {
                                 ...
                                 if (descriptionElement.length) {
                                     ...
                                     // If description element isn't ready, Lampa might use the overview property later
                         ...
                         if (streamData && Array.isArray(streamData.streams)) {
                             ...
                             var lampaStreams = streamData.streams.map(function(stream) {
                                 return {
                                     ...
                                     headers: stream.behaviorHints && stream.behaviorHints.proxyHeaders && stream.behaviorHeaders.proxyHeaders.request ? stream.behaviorHints.proxyHeaders.request : undefined
                                 };
                             });
                             ...
                             // Sometimes triggering a component update method (if available) or
                             // manually adding elements for playback options might be necessary.
                             // A common pattern is to check if the 'play' button or source list exists
                             // and update it. However, setting `e.object.card.playlist` is the standard
                             // way to provide streams to Lampa's player.
