(function () {
    ...
    const TAG_SLUG_MAP = { 
        "ahegao": "Ахегао", "bdsm": "БДСМ", "big-boobs": "Большая грудь", "blow-job": "Минет", 
        "bondage": "Бондаж", "paizuri": "Пайзури", "censored": "С цензурой", "comedy": "Комедия", 
        "cosplay": "Косплей", "creampie": "Крем-пай", "dark-skin": "Темная кожа", "facial": "На лицо", 
        "fantasy": "Фэнтези", "filming": "Съемка", "footjob": "Футджоб", "futanari": "Футанари", 
        "gangbang": "Гэнгбэнг", "glasses": "В очках", "harem": "Гарем", "hd": "HD", "horror": "Ужасы", 
        "incest": "Инцест", "inflation": "Раздувание", "lactation": "Лактация", 
        "small-boobs": "Маленькая грудь", "maids": "Горничные", "masturbation": "Мастурбация", 
        "milf": "Милфы", "mind-break": "Свести с ума", "mind-control": "Контроль сознания", 
        "monster-girl": "Монстры (девушки)", "neko": "Неко", "ntr": "НТР", "nurses": "Медсестры", 
        "orgy": "Оргия", "plot": "С сюжетом", "pov": "От первого лица", "pregnant": "Беременные", 
        "public-sex": "Публичный секс", "rape": "Изнасилование", 
        "reverse-rape": "Обратное изнасилование", "scat": "Дерьмо", "schoolgirls": "Школьницы", 
        "shota": "Шота", "ero": "Эротика", "swimsuit": "Купальник", "teacher": "Учитель", 
        "tentacles": "Тентакли", "threesome": "Тройничок", "toys": "Игрушки", "tsundere": "Цундере", 
        "ugly-bastard": "Противный ублюдок", "uncensored": "Без цензуры", "vanilla": "Классика", 
        "virgin": "Девственность", "watersports": "Золотой дождь", "x-ray": "X-ray", "yuri": "Юри",
    };
    ...
    function PluginCard(data, userLang) {
        const pr = {
            ...
            type_display: data.type === "series" ? "SERIES" : (data.type === "movie" ? "MOVIE" : (data.type ? data.type.toUpperCase() : "MOVIE"))
        };
    ...
    function PluginComponent(object) {
        ...
        this.currentCatalogKey = (this.activity.params && this.activity.params.catalog_key) ? 
                                     this.activity.params.catalog_key : 'new-releases';
        
        let currentCatalogConfig = API_CATALOG_CONFIG[this.currentCatalogKey];
        let isTagCatalog = false;
        let currentTagSlug = '';

        if (this.currentCatalogKey.startsWith('tag__')) {
            isTagCatalog = true;
            currentTagSlug = this.currentCatalogKey.substring(5); 
            currentCatalogConfig = { paginated: true, title_key_suffix: 'tag_' + currentTagSlug }; // Используем title_key_suffix для тегов
        } else if (!currentCatalogConfig) { 
             this.currentCatalogKey = 'new-releases';
             currentCatalogConfig = API_CATALOG_CONFIG[this.currentCatalogKey];
        }
        ...
        this.fetchData = function (page_to_fetch, onSuccess, onError, isAutoRetry = false) {
            ...
            network.native(urlToFetch, (responseData) => {
                ...
                if (onError) onError(getLangText('error_fetch_data', "Ошибка загрузки данных."));
        ...
        this.appendCardsToDOM = function (metasToAppend, originalApiBatchLength, isEmptyAfterFilter = false) {
            if (currentCatalogConfig && currentCatalogConfig.paginated) {
            ...
            if (isEmptyAfterFilter && can_load_more && auto_load_attempts < MAX_AUTO_LOAD_ATTEMPTS) {
                console.log("Plugin: Page empty after filtering, auto-loading next (attempt " + (auto_load_attempts + 1) + ").");
                ...
                const fallbackText = isTagCatalog ? (TAG_SLUG_MAP[currentTagSlug] || currentTagSlug) : (CATALOG_TITLES_FALLBACK[this.currentCatalogKey] || this.currentCatalogKey);
                const catTitle = getLangText(titleKeySuffix, fallbackText);
                ...
                card_render.on("hover:enter", () => {
                    ...
                    network.native(streamsUrl, (flaskResponse) => { 
                        ...
                        if (flaskResponse && flaskResponse.streams && flaskResponse.streams.length > 0) {
                            const playerItems = flaskResponse.streams.map(stream => {let st = stream.name||"Player"; if(stream.title)st+=` - ${stream.title}`; return {title:st,stream_details:stream}});
                            Lampa.Select.show({title:getLangText('player_select_title','Выберите плеер'),items:playerItems,onBack:()=>Lampa.Controller.toggle('content'),
                                onSelect:(si)=>{const sd=si.stream_details;const pld={title:cardFlaskData.name||'Без названия',poster:cardFlaskData.poster||'',id:cardFlaskData.id,name:cardFlaskData.name,type:cardFlaskData.type==='series'?'tv':'movie'};
                                    if(sd.url){let vu=sd.url;let uvp=true;if(uvp){vu=PROXY_FOR_EXTERNAL_URLS+encodeURIComponent(vu);if(Lampa.Noty)Lampa.Noty.show(getLangText('proxy_loading_notification','Загрузка через прокси...'),{time:1500})}pld.url=vu;if(Lampa.Timeline&&typeof Lampa.Timeline.view==='function')pld.timeline=Lampa.Timeline.view(pld.id)||{};if(Lampa.Timeline&&typeof Lampa.Timeline.update==='function')Lampa.Timeline.update(pld);Lampa.Player.play(pld);Lampa.Player.playlist([pld]);if(Lampa.Favorite&&typeof Lampa.Favorite.add==='function')Lampa.Favorite.add('history',pld);card.updateIcons()}
                    ...
                    }, (es,ed)=>{this.activity.loader(false);console.error("P:Err fetch streams",es,ed);Lampa.Noty.show(getLangText('player_streams_fetch_error','Ошибка загрузки плееров'))},false,{dataType:'json',timeout:20000});
                });
        ...
        this.loadNextPage = function(isAutoRetry = false) {
            ...
            if (this.currentCatalogKey === 'all' || (currentCatalogConfig && !currentCatalogConfig.paginated) ) {can_load_more = false; return;}
            if (!isAutoRetry) auto_load_attempts = 0; 
            current_api_page++; 
            ...
            this.fetchData(1, 
                (initialMetas, originalLength, isEmptyAfterFilterOnInit) => {
                    if (initialMetas.length > 0 || items_instances.length > 0) { 
                        ...
                        console.log("Plugin: Initial page empty after filtering, attempting to load next.");
        ...
        this.create = function () {
            const titleKeySuffix = isTagCatalog ? 'tag_' + currentTagSlug : 'cat_' + this.currentCatalogKey;
            const fallbackText = isTagCatalog ? (TAG_SLUG_MAP[currentTagSlug] || currentTagSlug) : (CATALOG_TITLES_FALLBACK[this.currentCatalogKey] || this.currentCatalogKey);
            const initialTitle = getLangText(titleKeySuffix, fallbackText);
        ...
        this.start = function () {
            ...
            const titleKeySuffix = isTagCatalog ? 'tag_' + currentTagSlug : 'cat_' + this.currentCatalogKey;
            const fallbackText = isTagCatalog ? (TAG_SLUG_MAP[currentTagSlug] || currentTagSlug) : (CATALOG_TITLES_FALLBACK[this.currentCatalogKey] || this.currentCatalogKey);
            const currentActivityTitle = getLangText(titleKeySuffix, fallbackText);
