(function () {
    ...
    function PluginCard(data, userLang) {
        ...
        const updateFavoriteIcons = () => {item.find('.lmeshm-card__fav-icons').remove(); const fc=$('<div class="lmeshm-card__fav-icons"></div>'); let fi=data.id; let ft=data.type==="series"?"tv":"movie"; if(data.id.includes('::')&&data.type==="series")fi=data.id.split('::')[1]; const cd={id:fi,title:data.name,name:data.name,poster:data.poster,year:data.year||'',type:ft,original_name:data.original_name||''}; const s=(Lampa.Favorite&&typeof Lampa.Favorite.check==='function'?Lampa.Favorite.check(cd):{})||{}; if(s.book)fc.append($('<div>').addClass('card__icon icon--book')); if(s.like)fc.append($('<div>').addClass('card__icon icon--like')); if(s.wath)fc.append($('<div>').addClass('card__icon icon--wath')); if(s.history||(Lampa.Timeline&&typeof Lampa.Timeline.watched==='function'&&Lampa.Timeline.watched(cd)))fc.append($('<div>').addClass('card__icon icon--history')); item.find('.LMEShikimori.card__view').append(fc);};
    ...
    function PluginComponent(object) {
        ...
        let currentCatalogConfig = API_CATALOG_CONFIG[this.currentCatalogKey]; // Может быть undefined для search или tag
        ...
        this.appendCardsToDOM = function (metasToAppend, originalApiBatchLength, isEmptyAfterFilter = false) {
            ...
            metasToAppend.forEach(meta => {
                ...
                card_render.on("hover:enter", () => {
                    const d=card.getRawData();let is=d.id;let itfs="hentai";if(d.id.includes('::')){const p=d.id.split('::');is=p[1]}const su=STREAM_ENDPOINT_TPL.replace('{type}',itfs).replace('{id}',is);this.activity.loader(true);network.clear(); 
                    network.native(su,fr=>{this.activity.loader(false);if(fr&&fr.streams&&fr.streams.length>0){const pi=fr.streams.map(s=>{let st=s.name||"P";if(s.title)st+=` - ${s.title}`;return{title:st,stream_details:s}});Lampa.Select.show({title:getLangText('player_select_title','Выберите плеер'),items:pi,onBack:()=>Lampa.Controller.toggle('content'),
                        onSelect:si=>{const sd=si.stream_details;const pld={title:d.name||'Без названия',poster:d.poster||'',id:d.id,name:d.name,type:d.type==='series'?'tv':'movie'};
        ...
        this.create = function () {
            let initialTitle;
            if (this.isSearchMode) {
                initialTitle = (getLangText('search_results_title', CATALOG_TITLES_FALLBACK.search_results_title)).replace('{query}', this.searchQuery);
            } else {
                const titleKeySuffix = isTagCatalog ? 'tag_' + currentTagSlug : 'cat_' + this.currentCatalogKey;
                const fallbackText = isTagCatalog ? TAG_SLUG_MAP[currentTagSlug] : CATALOG_TITLES_FALLBACK[this.currentCatalogKey];
                initialTitle = getLangText(titleKeySuffix, fallbackText || this.currentCatalogKey);
            }
        ...
        this.start = function () {
            ...
            if (this.isSearchMode) {
                currentActivityTitle = (getLangText('search_results_title', CATALOG_TITLES_FALLBACK.search_results_title)).replace('{query}', this.searchQuery);
            } else {
                const titleKeySuffix = isTagCatalog ? 'tag_' + currentTagSlug : 'cat_' + this.currentCatalogKey;
                const fallbackText = isTagCatalog ? TAG_SLUG_MAP[currentTagSlug] : CATALOG_TITLES_FALLBACK[this.currentCatalogKey];
                currentActivityTitle = getLangText(titleKeySuffix, fallbackText || this.currentCatalogKey);
            }
